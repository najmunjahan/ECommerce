...
namespace ECommerceProject.Controllers
{
    public class CartController : Controller
    {
        ...
        public JsonResult AddToCart(int pid, double qty)
        {
            ...
            if (extcart == null)
            {
                var prod = _context.Product.Include(m=>m.Unit).Include(x=> x.ProductImages).FirstOrDefault(x => x.Id == pid);
                if (prod != null)
                {
            ...
            {
                var prod1 = _context.Product.Include(m => m.Unit).Include(x=> x.ProductImages).FirstOrDefault(x => x.Id == pid);
                if (prod1 != null)
                {
        ...
        public JsonResult IncreaseItem(int pid)
        {
            ...
            if (extcart != null)
            {
                ...
                if (prod != null)
                {
                    prod.SaleQuantity += 1;
                    HttpContext.Session.SetObject<Cart>("mycart", extcart);
                    return Json(new { flag = "1", msg = "Product is increased successfully." });
                }
                else
                {
                    return Json(new { flag = "0", msg = "Product is invalid" });
            ...
            if (extcart != null)
            {
                ...
                if (prod != null)
                {
                    ...
                    if (qt >= 2)
                    {
                        prod.SaleQuantity -= 1;
                        HttpContext.Session.SetObject<Cart>("mycart", extcart);
                        return Json(new { flag = "1", msg = "Product is decreased successfully." });
                    }
                    else
                    {
